...
NetEase, QQ Music and Kugou must render through the same canonical MeloX Compose presentation wherever the product semantics are shared. A provider must not create a second Home, Explore, Search, Library, playlist-detail, player, animation, or background implementation just because its API is different.
...
This means a future visual improvement from MeloX iOS should normally be implemented once and automatically appear for QQ Music and Kugou when their capabilities can supply the required state.
...
The preferred pattern is one renderer consuming state plus capability-driven optional actions/sections.
